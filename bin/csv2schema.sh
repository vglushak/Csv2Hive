...
# The sed commands to transform the SQL-DDL file generated by csvsql into a simpler schema file
SED_COMMANDS="1d
$ d
s/\"//g
s/, /,/g
...
# Clean the DDL SQL file with the SED commands to obtain a simple schema file
# that will be used to build the Hive CREATE TABLE statement

while read -r command; do
  sed -i -- "$command" "${SCHEMA_FILE}"
done <<< "$SED_COMMANDS"
